...
router.post('/chatbot', async (req, res) => {
  try {
    ...
    try {
      ...
      const messageWords = normalizedMsg.split(' ').filter(w => w.length > 2); // Filtrer les mots trop courts
      
      // Recherche par mots-clés (priorité haute)
      ...
      // Recherche par texte dans la question (priorité moyenne)
      if (keywordMatches.length === 0) {
        const questionMatches = await ChatbotResponse.find({
          lang,
          isActive: true,
          question: { $regex: normalizedMsg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' }
        }).sort({ priority: -1, usageCount: -1 }).limit(1);

        if (questionMatches.length > 0) {
          customResponse = questionMatches[0];
        } else {
          // Recherche partielle dans la question (si la question contient des mots de la requête)
          const partialMatches = await ChatbotResponse.find({
            lang,
            isActive: true,
          ...
          if (partialMatches.length > 0) {
            customResponse = partialMatches[0];
          } else {
            // Dernière tentative : recherche dans la réponse
            const answerMatches = await ChatbotResponse.find({
              lang,
              isActive: true,
              answer: { $regex: normalizedMsg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' }
            }).sort({ priority: -1, usageCount: -1 }).limit(1);

            if (answerMatches.length > 0) {
              customResponse = answerMatches[0];
            }
          }
        }
      } else {
