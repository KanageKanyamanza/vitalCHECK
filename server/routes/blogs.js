...
const authenticateAdmin = async (req, res, next) => {
	try {
		...
		const admin = await Admin.findById(decoded.id || decoded.adminId).select(
			"-password",
		);
...
router.get("/:id/like/status", async (req, res) => {
	try {
		...
		if (userId) {
			...
			let existingLike = await BlogLike.findOne({
				blog: req.params.id,
				userId,
			});
	...
	try {
		...
		if (!userId && !visitorId) {
			return res.status(400).json({
				success: false,
				message:
					"Identifiant visiteur requis pour les utilisateurs non connectés",
			});
		}
...
router.post(
	...
	[
		...
		body("type")
			.isIn(["article", "etude-cas", "tutoriel", "actualite", "temoignage"])
...
router.put(
		...
		body("title")
			...
			.custom((value) => {
				if (value && typeof value === "object") {
					if (!value.fr?.trim() && !value.en?.trim()) {
						throw new Error(
							"Au moins un titre (français ou anglais) est requis",
						);
					}
				}
				return true;
		...
		body("content")
			...
			.custom((value) => {
				if (value && typeof value === "object") {
					if (!value.fr?.trim() && !value.en?.trim()) {
						throw new Error(
							"Au moins un contenu (français ou anglais) est requis",
						);
					}
				}
				return true;
	...
	async (req, res) => {
		try {
			...
			// Vérifier que l'admin est l'auteur ou a les permissions (super-admin ou admin)
			const isAdminOrSuperAdmin = ["admin", "super-admin"].includes(
				req.admin.role,
			);
			if (
				blog.author.toString() !== req.admin._id.toString() &&
				!isAdminOrSuperAdmin
			) {
			...
			if (updateData.title) {
				...
				if (
					updateData.title.fr &&
					updateData.title.fr !== blog.title?.fr &&
					!updateData.slug.fr
				) {
				...
				if (
					updateData.title.en &&
					updateData.title.en !== blog.title?.en &&
					!updateData.slug.en
				) {
...
router.get("/admin/blogs/:id/visits", authenticateAdmin, async (req, res) => {
	try {
		...
		const recentVisits = await BlogVisit.find({ blog: blog._id })
			...
			.limit(50)
			.select(
				"sessionId ipAddress country city device userAgent visitedAt timeOnPage scrollDepth isBounce referrerDomain utmSource utmMedium utmCampaign",
			);
