...
function generateV2HTMLContent(data, options = {}) {
	const { hidePremiumCTA = false } = options;
	const {
		companyName,
		companySize,
		sector,
...
const PREMIUM_TEXTS = {
	fr: {
		premiumTitle: 'Recommandations Personnalisées',
		premiumSubtitle: 'Plan d\'action approfondi généré par intelligence artificielle à partir de vos résultats de diagnostic',
		fallbackNote: 'Vos recommandations approfondies sont en cours de préparation. Elles seront disponibles lors de votre prochain téléchargement.',
		badge: '★ RAPPORT PREMIUM',
		continuity: 'Analyse approfondie · Suite du rapport',
	},
	en: {
		premiumTitle: 'Personalized Recommendations',
		premiumSubtitle: 'In-depth action plan generated by artificial intelligence from your diagnostic results',
		fallbackNote: 'Your in-depth recommendations are being prepared. They will be available on your next download.',
		badge: '★ PREMIUM REPORT',
		continuity: 'In-depth analysis · Report continuation',
	},
};
...
function generatePremiumPage(data, premiumInsights, language, textsRef) {
    ...
    <div class="page page-break">
      ...
      <!-- En-tête page 3 : même palette que le rapport -->
      <div class="header" style="margin-bottom:20px;">
        <div style="display:inline-block;background:rgba(255,255,255,0.25);color:white;font-size:10px;font-weight:800;letter-spacing:2px;padding:3px 12px;border-radius:20px;margin-bottom:8px;text-transform:uppercase;">${t.badge}</div>
        <h1 style="font-size:20px;font-weight:700;margin-bottom:4px;">${t.premiumTitle}</h1>
        <p style="font-size:12px;opacity:0.9;margin:0;">${t.premiumSubtitle}</p>
      </div>
      ...
      <!-- Bloc CTA (même style que dans le rapport standard) -->
      <div class="premium-section" style="margin-top:20px;">
        <h3>${std.benefitsTitle}</h3>
        <ul>
          ${std.benefits.map((b) => `<li>${b}</li>`).join('')}
        </ul>
        <a href="https://checkmyenterprise.com/checkout?plan=premium" class="premium-button">${std.ctaButton}</a>
      </div>

      <div class="footer-note" style="margin-top:12px;">
        <p>${std.footerNote}</p>
        <p>${std.generatedOn} ${formattedDate} — vitalCHECK · UBUNTU BUSINESS BUILDERS (UBB) – SARL · Dakar, Sénégal</p>
      </div>
    </div>`;
}
...
async function generatePremiumV2PDFReport(data, premiumInsights) {
	...
	// hidePremiumCTA=true : retire le bloc CTA de la page 2 (déplacé en fin de page 3)
	const standardHtml = generateV2HTMLContent(data, { hidePremiumCTA: true });
	const premiumPage = generatePremiumPage({}, premiumInsights, language, t);
