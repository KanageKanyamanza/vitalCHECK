...
export const generateClientPDF = async (assessment, elementId = 'pdf-content') => {
  try {
    ...
    // Télécharger le PDF
    const filename = `UBB-Health-Check-${user.companyName}-${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(filename);
...
const createPDFElement = (assessment) => {
  ...
  element.innerHTML = `
    <div style="text-align: center; margin-bottom: 30px;">
      <h1 style="color: #FF6B35; font-size: 28px; margin: 0;">UBB ENTERPRISE HEALTH CHECK</h1>
    ...
    <div style="margin-top: 50px; padding: 20px; background: #f8f9fa; text-align: center; border-radius: 8px;">
      <p style="margin: 0;">This report was generated by UBB Enterprise Health Check</p>
...
export const generateSimpleClientPDF = async (assessment) => {
  try {
    ...
    pdf.setFontSize(8);
    pdf.setTextColor(100, 100, 100);
    pdf.text('Généré par UBB Enterprise Health Check', 20, 280);
