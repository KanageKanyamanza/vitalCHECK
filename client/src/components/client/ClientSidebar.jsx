  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all group ${
        active
          ? "bg-[#F5A83C]/15 text-[#F5A83C] border-l-[3px] border-[#F5A83C] pl-[13px]"
          : "text-slate-300 hover:bg-white/8 hover:text-white border-l-[3px] border-transparent pl-[13px]"
      }`}
    >
      <Icon className={`w-4 h-4 shrink-0 ${active ? "text-[#F5A83C]" : "text-slate-400 group-hover:text-white"}`} />
      <span className="truncate">{label}</span>
      {badge && (
        <span className="ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded bg-[#F5A83C] text-[#13294B] shrink-0">
          {badge}
        </span>
      )}
    </button>
    ...
    <>
      ...
      <aside
        className={`
          ...
          bg-[#13294B] text-white shadow-2xl
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
          ...
          >
            ...
            <div className="leading-none">
              ...
              <br />
              <span className="text-[10px] text-[#F5A83C] font-medium uppercase tracking-widest">
                Enterprise Health
              </span>
            </div>
          </button>
        ...
        <div className="px-4 py-3 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-[#F5A83C] flex items-center justify-center shrink-0">
              <span className="text-[#13294B] font-bold text-sm">
            ...
            <div className="min-w-0">
              ...
              <span
                className={`text-[10px] font-bold uppercase tracking-wide ${
                  isPremium ? "text-[#F5A83C]" : "text-slate-400"
                }`}
              >
                {planLabel}
              </span>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          ...
          <NavItem
            icon={FileText}
            label={t("clientLayout.nav.diagnostics")}
            to="/client/dashboard"
            active={false}
            onClick={onClose}
          />
          <NavItem
            icon={CreditCard}
            label={t("clientLayout.nav.payments")}
            to="/client/dashboard"
            active={false}
            onClick={onClose}
          />
            ...
            {isPremium ? (
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium
                  text-slate-500 border border-dashed border-slate-600 hover:border-[#F5A83C]/50
                  hover:text-[#F5A83C]/70 transition-all group"
              >
