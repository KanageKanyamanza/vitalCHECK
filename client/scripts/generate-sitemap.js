...
async function generateClientSitemap() {
	try {
		console.log(
			"🚀 [SITEMAP CLIENT] Début de la génération du sitemap côté client...",
		);

		// URL de l'API pour récupérer les blogs
		const apiUrl =
			process.env.VITE_API_URL || "https://vitalcheck-brtv.onrender.com/api";
		...
		try {
			...
			console.log(
				`📝 [SITEMAP CLIENT] ${blogs.length} blogs publiés récupérés`,
			);
		} catch (error) {
			console.warn(
				"⚠️  [SITEMAP CLIENT] Impossible de récupérer les blogs depuis l'API:",
				error.message,
			);
			console.log(
				"📝 [SITEMAP CLIENT] Utilisation d'une liste de blogs par défaut",
			);
			// Liste de blogs par défaut si l'API n'est pas disponible
			blogs = [
				{
					slug: {
						fr: "du-cahier-au-digital-comment-structurer-vos-operations-sans-gros-budget",
					},
					updatedAt: new Date(),
				},
				{
					slug: {
						fr: "afrique-du-sud-le-leader-africain-du-march-des-aliments-emballs",
					},
					updatedAt: new Date(),
				},
				{
					slug: { fr: "5-signes-entreprise-diagnostic-sante" },
					updatedAt: new Date(),
				},
				{
					slug: { fr: "optimiser-gestion-tresorerie-5-etapes" },
					updatedAt: new Date(),
				},
				{
					slug: { fr: "temoignage-ubb-doubler-chiffre-affaires" },
					updatedAt: new Date(),
				},
				{
					slug: { fr: "5-signaux-alarme-diagnostic-urgent" },
					updatedAt: new Date(),
				},
				{
					slug: { fr: "etude-cas-transformation-digitale-pme" },
					updatedAt: new Date(),
				},
			];
		}
		...
		console.log(
			`   - Total URLs: ${staticPages.length + languagePages.length + blogCount}`,
		);
		...
		console.error(
			"❌ [SITEMAP CLIENT] Erreur lors de la génération du sitemap:",
			error,
		);
		process.exit(1);
	}
